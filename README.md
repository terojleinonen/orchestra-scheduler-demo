...
The UI is in Finnish (`<html lang="fi">`). Dates and times use `LOCALE = "fi-FI"` in `server/src/config.ts`;
other server-side texts (department and work type names, labels) are in `server/src/services/labels.ts`.
